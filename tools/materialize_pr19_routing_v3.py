#!/usr/bin/env python3
"""Wrapper PR19 v3: prioriza escapes locales críticos antes de feedback/PGOOD."""
...
class RouterV3(base.Router):
    def route(self) -> dict:
        ...
        def key(net: str):
            cls = self.by_net[net]
            local = 0 if routes_by_net[net]["mode"].startswith("LOCAL") else 1
            rank = NET_RANK.get(net, 1000)
            return (class_priority[cls], local, rank, net)
