#!/usr/bin/env python3
"""Micro-DRC de escapes U_EFUSE.5/.6 con el DRU real del proyecto."""
...
VARIANTS={
  ...
  'BOTH_UP_SPLIT': [
      ('12V_IN_RAW',(177.525,17.975),(177.525,13.5),0.25),
      ('12V_PROTECTED',(178.025,17.975),(178.025,15.25),0.25),
      ('12V_PROTECTED',(178.025,15.25),(180.25,15.25),0.25),
  ],
  'OUTWARD': [
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='pr20a_efuse_') as td:
        ...
        for name,tracks in VARIANTS.items():
            ...
            p=td/f'{name}.kicad_pcb'; r=td/f'{name}.json'; rules=td/f'{name}.kicad_dru'
            pcbnew.SaveBoard(str(p),b); shutil.copyfile(DRU,rules)
            cp=subprocess.run(['kicad-cli','pcb','drc',str(p),'--format','json','--output',str(r),'--severity-all'],text=True,capture_output=True)
            if not r.exists(): results[name]={'cli_rc':cp.returncode,'fatal':cp.stderr[-1000:]}; continue
            d=json.loads(r.read_text(encoding='utf-8')); errs=[v for v in d.get('violations',[]) if v.get('severity')=='error']
            results[name]={'cli_rc':cp.returncode,'errors':len(errs),'error_types':dict(Counter(str(v.get('type','?')) for v in errs)),'warnings':sum(v.get('severity')=='warning' for v in d.get('violations',[])),'unconnected':len(d.get('unconnected_items',[])),'first_errors':errs[:8]}
    print('EFUSE_ESCAPE_DRC_BEGIN'); print(json.dumps(results,ensure_ascii=False,separators=(',',':'))); print('EFUSE_ESCAPE_DRC_END')
    good=[k for k,v in results.items() if v.get('errors')==0]
    print('EFUSE_ESCAPE_GOOD',good)
    if not good: raise SystemExit('ERROR: ninguna variante de escape eFuse quedó DRC=0 con DRU real')
