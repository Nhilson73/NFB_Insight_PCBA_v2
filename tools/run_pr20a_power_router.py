#!/usr/bin/env python3
"""Ajustes de ejecución del router PR20A durante convergencia geométrica."""
...
def prepare_escape(self,n,e,w,role):
    ...
    be,_=self.build_blocked(n,ew); bf,_=self.build_blocked(n,w)
    start=(m.gc(e['x']),m.gc(e['y'])); q=deque([(start,0)]); prev={start:None}; goal=None
    # El portal solo necesita liberar el ancho completo en F.Cu. La vía se
    # selecciona después por A* con su propio keepout atravesando todas capas.
    while q:
        (ix,iy),d=q.popleft()
        if d>=3 and (ix,iy) not in bf[pcbnew.F_Cu]:
            goal=(ix,iy); break
        if d>=48: continue
    ...
    if goal is None: m.fail(f'sin portal escape {n} {e["ref"]}.{e["pad"]} w={w} ew={ew}')
    pts=[]; c=goal
    ...
    x,y=m.xy(*goal)
    return {'net':n,'ref':e['ref']+'_PORTAL','pad':e['pad'],'obj':None,'x':x,'y':y,'sx':w,'sy':w,'virtual':True,'layer':pcbnew.F_Cu,'source':m.pad_key(e['ref'],e['pad']),'escape_width':ew}
