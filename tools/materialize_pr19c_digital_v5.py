...
class RouterV5(v4.RouterV4):
    def _astar(self, net: str, a: dict, z: dict):
        ...
        if net=='I2C_SCL' and refs=={'D_GNSS_SCL','R_I2C_SCL'}:
            d=a if a['ref']=='D_GNSS_SCL' else z
            r=z if z['ref']=='R_I2C_SCL' else a
            pts=[
                (d['x_mm'],d['y_mm']),
                (133.75,17.00),
                (133.75,15.75),
    ...
    def _materialize(self, net: str, path, a: dict, z: dict):
        clsinfo=self.class_info[self.class_by_net[net]]
        ...
        def add(layer,p0,p1):
            ...
            self._add_track(net,layer,width,p0,p1)
            self._mark_track_cells(self.track_occ,net,layer,p0,p1,halo=1)
        ...
        for i in range(1,len(path)+1):
            ...
            layer=path[run][2]
            pts=[(path[j][0],path[j][1]) for j in range(run,i)]
            if len(pts)>=2:
                keep=[pts[0]]
                for k in range(1,len(pts)-1):
                    prev=pts[k-1]; cur=pts[k]; nxt=pts[k+1]
                    d1=(cur[0]-prev[0],cur[1]-prev[1]); d2=(nxt[0]-cur[0],nxt[1]-cur[1])
                    if d1==d2:
                        continue
                    keep.append(cur)
                keep.append(pts[-1])
                bends += max(0,len(keep)-2)
...
def main() -> int:
    ...
    r=RouterV5(board,placement,routing,batches)
    ...
    print('PR19C_CANDIDATE_V5',len(manifest['target_nets']),manifest['new_segment_count'],manifest['new_via_count'])
    return 0
