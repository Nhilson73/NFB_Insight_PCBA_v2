#!/usr/bin/env python3
"""Valida readiness físico PR16 antes de introducir coordenadas XY de producción."""
from __future__ import annotations
import json,re
from pathlib import Path
ROOT=Path(__file__).resolve().parents[1]
C=ROOT/'hardware/placement_readiness_contract.json'
PCB=ROOT/'kicad/NFB_Insight_PCBA_v2.kicad_pcb'
ROOTEDA=ROOT/'hardware/root_eda_contract.json'
...
def main():
    ...
    if not close(rf['derived_separation_from_host_xmax_to_z3_mm'],145.0-53.34) or not close(rf['derived_separation_from_host_xmax_to_z4_mm'],180.0-53.34): fail('separación quiet→dirty incorrecta')
    mech=c['mechanical']
    if not close(mech['board_height_mm'],68.58) or not close(mech['board_width_mm_current'],220.0) or mech['growth_direction']!='+X' or mech['field_io_direction']!='-Y': fail('mecánica base cambió')
    ...
    if zones['Z0']['x_max']!=53.34 or zones['Z3']['x_min']!=145.0 or zones['Z4']['x_min']!=180.0: fail('guías quiet/dirty cambiaron')
    li=c['layer_intent']
    ...
    refs=[x['ref'] for x in seq]
    expected=['J_PH','J_ORP','J_TEMP','U_CO2','J_DO','J_LOADCELL','J_GNSS_RTC','J_HMI','J_PWR_IN','J_PUMP','J_CO2_SOL','J_CHILLER_CTL']
    ...
    data={z:json.loads(p.read_text(encoding='utf-8')) for z,p in FILES.items()}
    comps={}
    ...
    footprint_refs=re.findall(r'\(property "Reference" "([^"]+)"',pcb)
    if footprint_refs!=['J_UNOQ']: fail(f'PR16 debe seguir sin placement de producción: {footprint_refs}')
    if '(segment ' in pcb or '(via ' in pcb or '(arc ' in pcb and '(layer "F.Cu")' in pcb: fail('routing no permitido en PR16')
    ...
    rules=c['placement_rules']
    required_true=['connectors_field_edge_first_row','esd_and_input_protection_immediately_behind_field_connectors','analog_conditioning_stays_in_Z1','loadcell_hx711_cluster_stays_on_Z1_SIDE_OF_Z2','hmi_stays_on_Z3_SIDE_OF_Z2','power_entry_tvs_efuse_cluster_stays_in_Z3','buck_switching_loop_stays_in_Z3','actuator_drivers_and_local_bulk_stay_in_Z4','actuator_connectors_stay_at_field_edge','pump_current_adc_keep_away_from_switch_nodes','high_current_returns_must_not_cross_Z1_Z2']
    if any(rules.get(k) is not True for k in required_true): fail('alguna regla EMC/placement fue debilitada')
    if rules.get('exact_xy_coordinates_frozen_in_pr16') is not False or rules.get('production_footprints_inside_Z0')!='FORBIDDEN': fail('PR16 adelantó XY o habilitó Z0')
    if 'NO MAINS' not in rules.get('chiller_contact_clearance_policy',''): fail('frontera chiller perdió NO MAINS')
    print('OK: PR16 pre-placement readiness cerrado')
    print('- Arduino docs-content snapshot actual registrado; RF numeric keepout no inventado')
    print('- Z0 protegido; Z1 quiet adyacente; Z3/Z4 ruidosos >=91.66/126.66 mm del borde +X del host')
    print('- 12 FIELD I/O ordenados; J_LOADCELL=Phoenix 1757268; placement actual=0; routing=0')
    return 0
