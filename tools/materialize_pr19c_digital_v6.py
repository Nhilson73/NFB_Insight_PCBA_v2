...
CANDIDATE_REVISION='v6-z4-corridors-first-active'
    ...
    def route_all(self) -> dict:
        ...
        for net in order:
            ...
            if net=='UNO_IOREF_3V3':
                ...
                edges=core.mst_edges(eps); edges.sort(key=lambda e:(round(v5.v4.edge_len(eps,e),6),e[0],e[1]))
                for i,j in edges:
        ...
        return {'schema_version':1,'status':'PR19C_DIGITAL_ROUTING_CANDIDATE','candidate_revision':CANDIDATE_REVISION,
                ...
                'planner':{'grid_mm':core.STEP,'compression':'COLLINEAR_GRID_RUNS_FIXED','priority':'Z4_LONG_HAUL_BEFORE_Z2_LONG_HAUL',
                           'preferred_lanes_mm':v5.v4.LANES,'micro_escape_i2c_scl':'D_GNSS_SCL.1 -> y15.75 -> R_I2C_SCL.2'}}
