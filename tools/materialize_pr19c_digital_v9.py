...
CANDIDATE_REVISION='v9-act-local-driver-escape-active'

class RouterV9(v8.RouterV8):
    def _astar(self,net,a,z):
        refs={a['ref'],z['ref']}
        if net=='ACT_FAULT_N' and refs=={'U_CO2_DRV','U_PUMP_DRV'}:
            pump=a if a['ref']=='U_PUMP_DRV' else z; co2=z if z['ref']=='U_CO2_DRV' else a
            pts=[(pump['x_mm'],pump['y_mm']),(213.00,17.75),(213.00,19.00),(216.75,19.00),(216.75,18.50),(co2['x_mm'],co2['y_mm'])]
            p=v5.grid_path(pts,pcbnew.F_Cu); return p if a['ref']=='U_PUMP_DRV' else list(reversed(p))
        return super()._astar(net,a,z)
    def route_all(self):
        r=super().route_all(); r['candidate_revision']=CANDIDATE_REVISION
        r.setdefault('planner',{})['act_fault_local_driver_escape']='U_PUMP_DRV.18 -> x213/y19 -> U_CO2_DRV.1 on F.Cu'; return r
