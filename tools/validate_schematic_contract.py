...
def main():
    for f in (PIN,SENSOR,Z1,Z2,Z2C,Z4,Z4C,POWER,ROOTEDA,MAT,PCB,SCH,Z1SCH,Z2SCH):
    ...
    rooteda=json.loads(ROOTEDA.read_text(encoding="utf-8")); mat=json.loads(MAT.read_text(encoding="utf-8"))
    if rooteda.get("schema_version")!=3 or rooteda.get("status")!="ROOT_EDA_PRODUCTION_MATERIALIZED_PR15": fail("root EDA no es PR15/schema3")
    scope=rooteda.get("scope",{})
    if scope.get("zone_internal_component_symbols") is not True or scope.get("placement") or scope.get("routing"): fail("scope root PR15 incorrecto")
    erc=rooteda.get("erc_policy",{})
    if erc.get("mode")!="ZERO_VIOLATIONS_REQUIRED_PR15" or erc.get("expected_errors")!=0 or erc.get("expected_warnings")!=0 or erc.get("expected_total_violations")!=0 or erc.get("severity_relaxation_allowed") is not False: fail("root PR15 no exige ERC cero")
    if mat.get("status")!="PRODUCTION_EDA_MATERIALIZED_PR15" or mat.get("erc_gate",{}).get("required_errors")!=0 or mat["erc_gate"].get("required_warnings")!=0: fail("contrato de materialización PR15 inválido")
    ...
    for marker in ("PR #14","Z0 — Arduino UNO Q Host","Z1 — Sensors","Z2 — Digital / Low Noise","Z3 — Power","Z4 — Actuators","uno_q_interface.kicad_sch","z1_interface.kicad_sch","z2_interface.kicad_sch","z3_interface.kicad_sch","z4_interface.kicad_sch"):
        if marker not in root: fail(f"root sin marcador jerárquico {marker}")
    for child in ("uno_q_interface.kicad_sch","z1_interface.kicad_sch","z2_interface.kicad_sch","z3_interface.kicad_sch","z4_interface.kicad_sch"):
        text=(ROOT/"kicad"/child).read_text(encoding="utf-8")
        if "NFB_GEN:" not in text or "PR #15 GENERATED" not in text: fail(f"{child} no está materializado PR15")
    ...
    print("OK: contrato global + root EDA materializado PR15 verificado")
    print("- Z0 participa en GND, no en 3V3/5V locales; A4/D10 Z4 preservados")
    print("- símbolos internos generados desde JSON/BOM; ERC contractual estricto = 0 errores / 0 warnings")
    return 0
