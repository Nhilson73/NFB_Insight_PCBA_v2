#!/usr/bin/env python3
"""Valida coherencia cruzada Z1 + Z2 + Z3 potencia + Z4 + UNO Q."""
...
def main():
    ...
    if not ic["scope"].get("actuator_connectivity") or ic["scope"].get("placement") or ic["scope"].get("routing"): fail("scope PR12 incorrecto")
    if audit.get("status")!="FOOTPRINT_AUDIT_EXTENDED_PR12": fail("integración no enlaza audit PR12")
    ...
    # Z4 must source 12V_ACT only from F_ACT branch and not power chiller.
    c4=compmap(z4)
    ...
    # Ownership Z1/Z2 remains only I2C overlap; Z4 owns dedicated actuator pins.
    z1_uno=set(z1["uno_q_interface"]["sensor_endpoints"].values())-{None}; z2_uno=set(z2["uno_q_interface"]["endpoints"].values()); overlap=z1_uno&z2_uno
    if overlap!={"I2C_SDA","I2C_SCL"}: fail(f"colisión Z1/Z2: {overlap}")
    ...
    for marker in ("J_UNOQ.4","J_UNOQ.5","CO2_ADC","A3","D9","IR2104","mains","placement","routing"):
        if marker not in prohib: fail(f"hard_prohibitions sin {marker}")
    ...
    for marker in ("PR #11","J_UNOQ.4 NO conecta","J_UNOQ.5 NO conecta","MPRLS0030PA00002A = 0x28","DFR1103 GNSS+RTC = 0x66","Routing permanece bloqueado"):
        if marker not in sch: fail(f"hoja integración PR11 perdió {marker}")
    print("OK: integración Z1+Z2+Z3+Z4 PR12 coherente; placement/routing bloqueados")
    return 0
