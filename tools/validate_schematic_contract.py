#!/usr/bin/env python3
"""Valida contrato UNO Q, Z1, Z2 y frontera de potencia después de PR #9."""
...
EXPECTED={
2:("IOREF","UNO_IOREF_3V3","ACTIVE_CONTROL_OUTPUT"),
3:("~RESET","MCU_NRST","ACTIVE"),
4:("3V3 OUT",None,"NC_HOST_POWER_OUT"),
5:("5V JANALOG",None,"NC_HOST_5V_SUPPORTED_INPUT"),
8:("VIN","12V_HOST_VIN","ACTIVE_POWER_INPUT"),
9:("A0","PH_ADC","ACTIVE"),10:("A1","ORP_ADC","ACTIVE"),
11:("A2/D16","TEMP_1WIRE","ACTIVE_DIGITAL"),12:("A3",None,"DNP_RESERVE"),
13:("A4",None,"DNP_RESERVE"),14:("A5","DO_ADC","ACTIVE"),
15:("D0","HMI_RX","ACTIVE"),16:("D1","HMI_TX","ACTIVE"),
17:("D2","HX711_DOUT","ACTIVE"),18:("D3","HX711_SCK","ACTIVE"),
19:("D4","MCU_WDI","ACTIVE"),20:("D5","PUMP_PWM","ACTIVE"),
21:("D6","PUMP_DIR","ACTIVE"),22:("D7","CO2_SOL_CTL","ACTIVE"),
23:("D8","CHILLER_CTL","ACTIVE_CONTROL_ONLY"),24:("D9",None,"DNP_RESERVE"),
25:("D10","RS485_IRQ_RSVD","RESERVE"),28:("D13","LED_STATUS","ACTIVE"),
31:("D20/SDA","I2C_SDA","ACTIVE"),32:("D21/SCL","I2C_SCL","ACTIVE")}
def fail(m): raise SystemExit("ERROR: "+m)
def main():
    for f in (PIN,SENSOR,Z1,Z2,Z2C,POWER,PCB,SCH,Z2SCH):
    ...
    if p.get("power_architecture_source_of_truth")!="hardware/power_architecture_contract.json": fail("falta declarar contrato potencia")
    ...
    if pi.get("host_5v",{}).get("official_capability") != "REGULATED_5V_HOST_INPUT_UP_TO_3A": fail("contrato no reconoce 5V JANALOG oficial")
    if pi.get("host_5v",{}).get("shield_connection") != "NC" or pi.get("host_3v3",{}).get("shield_connection") != "NC": fail("rails locales quedaron unidos al host")
    if not pi.get("backfeed_forbidden"): fail("backfeed no está prohibido")
    active={x.get("net") for x in p["pins"] if str(x.get("status","")).startswith("ACTIVE")}
    bad={"TEMP_ADC","CO2_ADC","HUM_ADC","CO2_PWM","CO2_FLOW_PWM","5V_RAIL","3V3_RAIL"} & active
    if bad: fail(f"nets prohibidas activas en interfaz host: {sorted(bad)}")
    ...
    if power["host_power"]["preferred_method_for_nfb"]!="VIN_7_TO_24V": fail("método host no es VIN")
    if not any("regulated 5 V" in x for x in power["host_power"]["official_supported_inputs"]): fail("contrato potencia oculta entrada 5V oficial")
    ...
    for nz,name in ((z1,"Z1"),(z2,"Z2")):
        ...
        if "J_UNOQ.4" in nets.get("3V3_RAIL",set()): fail(f"{name} une 3V3_RAIL a host")
        if "J_UNOQ.5" in nets.get("5V_RAIL",set()): fail(f"{name} une 5V_RAIL a host")
    ...
    print("OK: contrato UNO Q + Z1 + Z2 + potencia PR #9 verificado")
    print("- VIN 7-24V; 5V JANALOG oficial reconocido pero no unido al 5V_RAIL local")
    print("- 3V3/5V locales desacoplados del host; IOREF solo referencia/enable")
    print("- MPR 0x28 + DFR1103 0x66; 32 pads físicos intactos")
    return 0
