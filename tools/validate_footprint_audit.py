#!/usr/bin/env python3
"""Valida el cierre físico PR13 antes de habilitar placement."""
...
FPS={
 "RPW":FPDIR/"TI_RPW0010A_TPS259470A.kicad_mod",
 "RDN":FPDIR/"TI_RDN0011A_TPSM33625.kicad_mod",
 "RHL":FPDIR/"TI_RHL0020B_DRV8242.kicad_mod",
 "DYC":FPDIR/"TI_DYC0008A_TPS1HC120.kicad_mod",
 "AQY":FPDIR/"Panasonic_AQY212EHAX_DIP4_SMD.kicad_mod",
 "MPR":FPDIR/"Honeywell_MPR_LongPort_12Pad.kicad_mod"}
...
def main():
    ...
    expected_fp={
      "TPS25947_RPW0010A":"NFB:TI_RPW0010A_TPS259470A",
      "TPSM33625_RDN11":"NFB:TI_RDN0011A_TPSM33625",
      "DRV8242_RHL20":"NFB:TI_RHL0020B_DRV8242",
      "TPS1HC120_DYC8":"NFB:TI_DYC0008A_TPS1HC120",
      "AQY212EHAX_DIP4_SMD":"NFB:Panasonic_AQY212EHAX_DIP4_SMD"}
    ...
    # RPW HotRod: 10 números lógicos; cuatro esquinas deben usar dos primitives cada una.
    rpw=FPS["RPW"].read_text(encoding="utf-8")
    nums=[int(x) for x in re.findall(r'\(pad "(\d+)"',rpw)]
    ...
    # RDN: geometría vendor/drawing, grandes pads 4/5 con paste windowed y SMD mask.
    ...
    # RHL actual del DRV8242: revisión B, 20 señales + EP21; no revisión A.
    rhl=FPS["RHL"].read_text(encoding="utf-8")
    nums=[int(x) for x in re.findall(r'\(pad "(\d+)"',rhl)]
    ...
    # DYC: exactamente 8 pads, sin PowerPAD inventado.
    dyc=FPS["DYC"].read_text(encoding="utf-8")
    nums=[int(x) for x in re.findall(r'\(pad "(\d+)"',dyc)]
    ...
    # AQY exacto SMD X.
    aqy=FPS["AQY"].read_text(encoding="utf-8")
    nums=[int(x) for x in re.findall(r'\(pad "(\d+)"',aqy)]
    ...
    power=json.loads(POWER.read_text(encoding="utf-8")); z4=json.loads(Z4.read_text(encoding="utf-8"))
    pc={x["ref"]:x for x in power["components"]}; zc={x["ref"]:x for x in z4["components"]}
    ...
    # Cierre de footprints no es placement: PCB debe seguir sin estos refs.
    pcb=PCB.read_text(encoding="utf-8")
    placed=[r for r in actual if f'"{r}"' in pcb]
    if placed: fail(f"PR13 no debe hacer placement: {placed}")
    ...
    print("- PCB sin placement: habilitación física será PR posterior")
    return 0
