...
core.STEP=0.25
core.PAD_HALO=0.45
CANDIDATE_REVISION='v8-conservative-clearance-model-active'
...
class RouterV8(v7.RouterV7):
    ...
    def route_all(self):
        ...
        r.setdefault('planner',{}).update({'pad_halo_mm':core.PAD_HALO,'track_cell_halo':2,'via_cell_halo':3,
            'clearance_model':'CONSERVATIVE_POSITIVE_MARGIN','act_fault_corridor':'B y9.5 with F bypasses x89-99, x104.5-112.5, x157.5-161; B rise x196'})
        return r
...
def main()->int:
    ...
    r=RouterV8(board,placement,routing,batches); manifest=r.route_all(); core.OUT.write_text(json.dumps(manifest,indent=2,ensure_ascii=False)+'\n',encoding='utf-8'); pcbnew.SaveBoard(str(core.PCB),board)
