#!/usr/bin/env python3
"""Valida contrato, BOM, netlist y ERC prerequisites de Z4 PR12."""
from __future__ import annotations
import csv,json,math
from pathlib import Path
ROOT=Path(__file__).resolve().parents[1]
C=ROOT/"hardware"/"z4_actuator_contract.json"; N=ROOT/"hardware"/"z4_production_netlist.json"; P=ROOT/"hardware"/"insight_pin_contract.json"; A=ROOT/"hardware"/"footprint_audit.json"; B=ROOT/"bom"/"insight_z4_production_bom.csv"; S=ROOT/"kicad"/"z4_actuators.kicad_sch"; PCB=ROOT/"kicad"/"NFB_Insight_PCBA_v2.kicad_pcb"
...
def main():
    ...
    if n.get("schema_version")!=1 or n.get("status")!="FROZEN_Z4_NETLIST_PR12": fail("netlist Z4 no es PR12")
    ...
    if c["design_policy"]["placement_in_scope"] or c["design_policy"]["routing_in_scope"] or n["placement_in_scope"] or n["routing_in_scope"]: fail("PR12 no debe habilitar placement/routing")
    ...
    if not close(vnom,t["nominal_adc_v_typ"],5e-4) or not close(imax,t["max_current_for_3p05v_using_min_ratio_a"],5e-4): fail("cálculo IPROPI almacenado incorrecto")
    if not (vnom<1.0 and imax>2.7): fail("telemetría bomba sin margen ADC esperado")
    co=c["co2_solenoid"]
    ...
    if not close(ilim,0.5,1e-5) or not close(co["current_limit"]["target_a"],0.5): fail("R_ILIM CO2 no produce ~0.5A")
    if co["driver"]["external_flyback_diode"]!="NOT_POPULATED_BASELINE": fail("no debe poblarse flyback legacy")
    ch=c["chiller"]
    ...
    if diag["net"]!="ACT_FAULT_N" or diag["uno_q_pad"]!=25 or diag["pullup_ohm"]!=10000 or set(diag["contributors"])!={"U_PUMP_DRV.nFAULT","U_CO2_DRV.FLT1"}: fail("bus diagnóstico incorrecto")
    ...
    for aid,ref in (("DRV8242_RHL20","U_PUMP_DRV"),("TPS1HC120_DYC8","U_CO2_DRV"),("AQY212EHAX_DIP4_SMD","U_CHILLER")):
        if a[aid]["placement_allowed"] is not False: fail(f"audit {aid} no bloquea placement")
        if not str(comps[ref]["footprint"]).startswith("PENDING_"): fail(f"{ref} perdió placeholder audit")
    pcb=PCB.read_text(encoding="utf-8"); placed=[r for r in comps if f'"{r}"' in pcb]
    if placed: fail(f"PR12 no debe colocar Z4: {placed[:5]}")
    ...
    for ref in comps:
        if ref not in sch: fail(f"schematic Z4 no indexa {ref}")
    print(f"OK: Z4 PR12 — {len(comps)} refs, BOM/netlist coinciden, placement=0")
    print(f"- Pump telemetry: {vnom:.3f}V @0.8A; ~{imax:.2f}A at 3.05V worst scale")
    print("- CO2 smart high-side ~0.5A; chiller PhotoMOS SELV <=48V")
    return 0
